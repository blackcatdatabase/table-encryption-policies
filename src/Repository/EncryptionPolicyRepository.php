...
use BlackCat\Database\Packages\EncryptionPolicies\ContractRepository;

final class EncryptionPolicyRepository implements ContractRepository {
    ...
    public function insert(array $row): void {
        ...
        $sql = "INSERT INTO encryption_policies (".implode(',', $cols).") VALUES (".implode(',', $place).")";
    ...
    public function insertMany(array $rows): void {
        ...
        $sql = "INSERT INTO encryption_policies (".implode(',', $cols).") VALUES ".implode(',', $allPlace);
        $this->db->execute($sql, $params);
    }
    ...
    public function upsert(array $row): void {
        ...
        $sqlBase = "INSERT INTO encryption_policies (".implode(',', $cols).") VALUES (".implode(',', $place).")";
    ...
    public function updateById(int|string $id, array $row): int {
        ...
        $sql = "UPDATE encryption_policies SET ".implode(',', $assign)." WHERE id = :id".$verCond;
    ...
    public function deleteById(int|string $id): int {
        ...
        if ($soft) {
            ...
            return $this->db->execute("UPDATE encryption_policies SET $set WHERE id = :id", $params);
        }
        return $this->db->execute("DELETE FROM encryption_policies WHERE id = :id", [':id'=>$id]);
    }
    ...
    public function restoreById(int|string $id): int {
        ...
        $set = "$soft = NULL";
        if ($updAt && $updAt !== $soft) { $set .= ", $updAt = CURRENT_TIMESTAMP"; }
        return $this->db->execute("UPDATE encryption_policies SET $set WHERE id = :id", [':id'=>$id]);
    }

    // ============ READ / PAGE / LOCK ============
    public function findById(int|string $id): ?array {
        $sql = "SELECT * FROM encryption_policies WHERE id = :id";
        $row = $this->db->fetchOne($sql, [':id'=>$id]);
        return $row ?: null;
    }

    public function exists(string $whereSql = '1=1', array $params = []): bool {
        $sql = "SELECT 1 FROM encryption_policies WHERE $whereSql LIMIT 1";
        return (bool)$this->db->fetchOne($sql, $params);
    }

    public function count(string $whereSql = '1=1', array $params = []): int {
        $sql = "SELECT COUNT(*) FROM encryption_policies WHERE $whereSql";
    ...
    public function paginate(Criteria $c): array {
        ...
        $order = $order ?: (Definitions::defaultOrder() ?? 'id DESC');
        $joins = $joins ?: '';

        $total = (int)$this->db->fetchOne("SELECT COUNT(*) FROM encryption_policies t $joins WHERE $where", $params);
        $items = $this->db->fetchAll("SELECT t.* FROM encryption_policies t $joins WHERE $where ORDER BY $order LIMIT $limit OFFSET $offset", $params);
    ...
    public function lockById(int|string $id): ?array {
        $sql = $this->db->isMysql()
            ? "SELECT * FROM encryption_policies WHERE id = :id FOR UPDATE"
            : "SELECT * FROM encryption_policies WHERE id = :id FOR UPDATE";
