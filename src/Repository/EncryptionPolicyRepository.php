...
namespace BlackCat\Database\Packages\EncryptionPolicies\Repository;

use BlackCat\Core\Database as Database;              // e.g. BlackCat\Core\Database
use BlackCat\Database\Packages\EncryptionPolicies\Definitions;
use BlackCat\Database\Packages\EncryptionPolicies\Criteria;
use BlackCat\Database\Packages\EncryptionPolicies\Dto\EncryptionPolicyDto as Dto;
use BlackCat\Database\Packages\EncryptionPolicies\Mapper\EncryptionPolicyDtoMapper as RowMapper;
...
final class EncryptionPolicyRepository implements RepoContract, KeysetRepoContract
{
     ...
     */
    protected function def(): string { return \BlackCat\Database\Packages\EncryptionPolicies\Definitions::class; }
    ...
    private function doUpsert(array $row, bool $revive): void
    {
        ...
        $updCols = [ 'mode', 'layer_selection', 'min_layers', 'max_layers', 'aad_template', 'notes' ];
    ...
    public function upsertMany(array $rows): int {
        ...
        if (class_exists(\BlackCat\Database\BulkUpsertRepository::class) && [ 'policy_name' ] !== []) {
            $bulk = new \BlackCat\Database\BulkUpsertRepository($this->db, \BlackCat\Database\Packages\EncryptionPolicies\Definitions::table());
            return $bulk->upsertMany($rows, [ 'policy_name' ], [ 'mode', 'layer_selection', 'min_layers', 'max_layers', 'aad_template', 'notes' ]);
        }
    ...
    */
    ...
    /** @return array<string,mixed>|\BlackCat\Database\Packages\EncryptionPolicies\Dto\EncryptionPolicyDto|null */
    public function getByPolicyName(string $policyName, bool $asDto = false): array|\BlackCat\Database\Packages\EncryptionPolicies\Dto\EncryptionPolicyDto|null {
        $row = $this->getByUnique([ 'policy_name' => $policyName ]);
        if (!$asDto || !$row) return $row;
        return \BlackCat\Database\Packages\EncryptionPolicies\Mapper\EncryptionPolicyDtoMapper::fromRow($row);
    }
    public function existsByPolicyName(string $policyName): bool {
        $where = 't.' . Ident::q($this->db, 'policy_name') . ' = :uniq_policy_name';
        return $this->exists($where, [ 'uniq_policy_name' => $policyName ]);
    }
    /** @return int|string|null */
    public function getIdByPolicyName(string $policyName) {
        $row = $this->getByPolicyName($policyName);
        return $row ? ($row['id'] ?? null) : null;
    }
    /** @return array<string,mixed>|\BlackCat\Database\Packages\EncryptionPolicies\Dto\EncryptionPolicyDto|null */
    public function getById(int $id, bool $asDto = false): array|\BlackCat\Database\Packages\EncryptionPolicies\Dto\EncryptionPolicyDto|null {
