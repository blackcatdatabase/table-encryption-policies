...
use BlackCat\Database\Packages\EncryptionPolicies\Repository\EncryptionPolicyRepositoryInterface;
...
class EncryptionPolicyRepository implements EncryptionPolicyRepositoryInterface, RepoContract, KeysetRepoContract
{
use OrderByTools, PkTools, RepositoryHelpers;

    /** @var mixed literal token for upsert keys (array or empty). */
    private mixed $tokenUpsertKeys = [ 'policy_name' ];
    ...
    public function upsertMany(array $rows): int {
        ...
        if ($helperKeys && class_exists(\BlackCat\Database\Support\BulkUpsertHelper::class)) {
          $bulk = new \BlackCat\Database\Support\BulkUpsertHelper($this->db, \BlackCat\Database\Packages\EncryptionPolicies\Definitions::class);
          $bulk->upsertMany($rows, $helperKeys, [ 'mode', 'layer_selection', 'min_layers', 'max_layers', 'aad_template', 'notes' ]);
          return count($rows);
        }
    ...
    public function getByPolicyName(string $policyName, bool $asDto = false): array|\BlackCat\Database\Packages\EncryptionPolicies\Dto\EncryptionPolicyDto|null {
        return $this->getByUnique([ 'policy_name' => $policyName ], $asDto);
    }
    ...
    public function getIdByPolicyName(string $policyName) {
        $row = $this->getByPolicyName($policyName, false);
