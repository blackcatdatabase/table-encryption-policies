 ...
 */
final class EncryptionPoliciesAggregateService
{
use ServiceHelpers;
    ...
    public function __construct(
        private Database $db, private EncryptionPolicyRepository $encryptionPolicyRepo
    ) {
        // Repository autowiring (generated)
        $this->repositories = [
  'encryption_policies' => $this->encryptionPolicyRepo
];
    }
    ...
    protected function withRowLock(callable $locker, int|string|array $id, callable $fetch, string $mode = 'wait'): mixed
    {
        ...
        return $this->txn(function () use ($locker, $id, $fetch, $mode) {
            ...
            if ($row === null) {
                ...
                throw new \BlackCat\Database\Packages\EncryptionPolicies\ModuleException("Record {$idStr} not found for locking.");
            }
