...
final class ContractRepository {
    ...
    public function findOne(string $where, array $params): ?array {
        $sql = "SELECT * FROM v_encryption_policies_contract WHERE $where LIMIT 1";
    ...
    public function list(string $where = "1=1", array $params = [], ?string $order = null, ?int $limit = null, ?int $offset = null): array {
        ...
        $sql = "SELECT * FROM v_encryption_policies_contract WHERE $where ORDER BY $order".$suffix;
    ...
    public function paginateView(Criteria $c): array {
        ...
        $order = $order ?: '1';
        $joins = $joins ?: '';
        $total = (int)$this->db->fetchOne("SELECT COUNT(*) FROM v_encryption_policies_contract v $joins WHERE $where", $params);
        $items = $this->db->fetchAll("SELECT v.* FROM v_encryption_policies_contract v $joins WHERE $where ORDER BY $order LIMIT $limit OFFSET $offset", $params);
