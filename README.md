...
> Lineage: 0 outbound / 4 inbound · ✅ No engine drift detected · Index coverage: todo · PII flags: 1 · Changelog: fresh (2 d)
...
| Inbound links (tables depending on this) | **4** |
| Views | **4** |
| Seeds | **0** |
| Drift warnings | **0** |
| PII flags | **1** |
...
| Lineage | outbound **0** / inbound **4** |
| Index coverage | _todo (add PK and at least one index)_ |
...
graph LR
  ...
  encryption_policies["encryption_policies"]:::center
  encryption_policy_bindings["encryption_policy_bindings"]:::inbound -->|FK| encryption_policies
  policy_algorithms["policy_algorithms"]:::inbound -->|FK| encryption_policies
  policy_kms_keys["policy_kms_keys"]:::inbound -->|FK| encryption_policies
  pq_migration_jobs["pq_migration_jobs"]:::inbound -->|FK| encryption_policies
...
- Inbound (relies on this): "encryption_policy_bindings", "policy_algorithms", "policy_kms_keys", "pq_migration_jobs"
...
| postgres | ✅ schema(2)<br/>✅ views(1)<br/>⚠️ seeds |
...
pwsh -NoLogo -NoProfile -File scripts/schema-tools/Migrate-DryRun.ps1 -Package encryption-policies -Apply
